...
ACTION_SPACE_SIZE = HAND_SIZE * ACTIONS_PER_HAND
...
class CustomCNNExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space, features_dim=256):
        ...
        board_height, board_width = board_shape[1], board_shape[2]
        ...
        spatial_output_size = 64 * board_height * board_width
        
        hand_flat_size = int(np.prod(hand_shape))
...
class CustomCNNExtractorV2(BaseFeaturesExtractor):
    def __init__(self, observation_space, features_dim=512):
        ...
        spatial_output_size = 96 * board_height * board_width
        hand_flat_size = int(np.prod(hand_shape))
        self.hand_fc = nn.Sequential(
            nn.Flatten(),
...
class ActionAwareCNNExtractor(BaseFeaturesExtractor):
    def __init__(self, observation_space, features_dim=768):
        ...
        coord_y = torch.linspace(-1.0, 1.0, board_height).view(1, 1, board_height, 1).expand(1, 1, board_height, board_width)
        coord_x = torch.linspace(-1.0, 1.0, board_width).view(1, 1, 1, board_width).expand(1, 1, board_height, board_width)
        ...
        self.slot_cnn = nn.Sequential(
            ...
            nn.GroupNorm(8, 128),
            nn.ReLU(),
            nn.AdaptiveAvgPool2d((board_height, board_width)),
            nn.Flatten(),
            nn.Linear(128 * board_height * board_width, 256),
            nn.ReLU(),
        )
        ...
        self.global_cnn = nn.Sequential(
            ...
            nn.AdaptiveAvgPool2d((board_height, board_width)),
            nn.Flatten(),
            nn.Linear(96 * board_height * board_width, 192),
            nn.ReLU(),
        )
    ...
    def forward(self, observations):
        ...
        for slot_idx in range(3):
            ...
            slot_embedding = self.slot_hand_encoder(slot_hand).view(batch_size, 24, 1, 1).expand(-1, -1, self.board_height, self.board_width)
            slot_available = available[:, slot_idx].view(batch_size, 1, 1, 1).expand(-1, 1, self.board_height, self.board_width)
...
class BlockBlastEnv(gym.Env):
    def __init__(
        ...
        self.actions_per_hand = self.board_size * self.board_size
        self.action_space_size = HAND_SIZE * self.actions_per_hand
        ...
        self.observation_space = spaces.Dict({
            "board": spaces.Box(low=0, high=1, shape=(1, self.board_size, self.board_size), dtype=np.int32),
            "valid_actions": spaces.MultiBinary((HAND_SIZE, self.board_size, self.board_size)),
            "hand": spaces.Box(low=0, high=1, shape=(HAND_SIZE, HAND_PAD_SIZE, HAND_PAD_SIZE), dtype=np.int32),
            ...
            for r in range(self.board_size):
                for c in range(self.board_size):
                    if self.game.can_place(block, r, c):
                        action_idx = (hand_idx * self.actions_per_hand) + (r * self.board_size) + c
                        mask[action_idx] = 1
        return mask
        ...
        def bfs_flood_fill(start_r, start_c):
            ...
            while queue:
                ...
                for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                    nr, nc = r + dr, c + dc
                    if 0 <= nr < self.board_size and 0 <= nc < self.board_size and not visited[nr, nc] and grid[nr, nc] == 0:
                        visited[nr, nc] = True
                        queue.append((nr, nc))
                        region_size += 1
    ...
    def _get_obs(self):
        padded_hand = np.zeros((HAND_SIZE, HAND_PAD_SIZE, HAND_PAD_SIZE), dtype=np.int32)
        for i, block in enumerate(self.game.hand):
            h, w = block.shape
            padded_hand[i, :h, :w] = block
        
        board_reshaped = self.game.grid.copy().reshape(1, self.board_size, self.board_size).astype(np.int32)
        
        return {
            "board": board_reshaped,
            "valid_actions": self.valid_action_mask().reshape(HAND_SIZE, self.board_size, self.board_size).astype(np.int8),
            "hand": padded_hand,
        ...
        for br in range(block_h):
            for bc in range(block_w):
                ...
                for dr, dc in [(-1, 0), (1, 0), (0, -1), (0, 1)]:
                    ...
                    nbr_r = board_r + dr
                    nbr_c = board_c + dc
                    if nbr_r < 0 or nbr_r >= self.board_size or nbr_c < 0 or nbr_c >= self.board_size:
                        touching_edges += 1
                        continue
    ...
    def step(self, action):
        hand_index = action // self.actions_per_hand
        remainder = action % self.actions_per_hand

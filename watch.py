...
from blocks import SHAPES, TRAINING_POOLS
...
def parse_args():
    ...
    parser.add_argument(
        "--model-path",
        default="checkpoints/roundsafe/block_blast_roundsafe_v1.zip",
        help="Model path with or without .zip",
    )
...
def create_env_bundle(args=None):
    raw_env = BlockBlastEnv(
        ...
        shape_pool=getattr(args, "shape_pool", "all"),
        hand_generator=getattr(args, "hand_generator", "adaptive_playable"),
    )
...
def in_bounds(block_shape, row, col):
    h, w = block_shape.shape
    if row < 0 or col < 0:
        return False
    if row + h > GRID_SIZE or col + w > GRID_SIZE:
        return False
    return True
...
class ArenaWidget(QWidget):
    ...
    def init_menu_scene(self):
        ...
        self.menu_buttons = [
            {
                "key": "solver",
                "label": "Let AI solve your position",
                "accent": HUMAN,
                "rect": QRect(base_x, base_y, MENU_BTN_W, MENU_BTN_H),
            },
            {
                "key": "watch",
                "label": "Watch AI play a game",
                "accent": AI,
                "rect": QRect(base_x, base_y + (MENU_BTN_H + MENU_BTN_GAP), MENU_BTN_W, MENU_BTN_H),
            },
            {
                "key": "heuristic",
                "label": "Watch heuristic play",
                "accent": WARN,
                "rect": QRect(base_x, base_y + 2 * (MENU_BTN_H + MENU_BTN_GAP), MENU_BTN_W, MENU_BTN_H),
            },
            {
                "key": "versus",
                "label": "You vs the AI",
                "accent": (175, 121, 255),
                "rect": QRect(base_x, base_y + 3 * (MENU_BTN_H + MENU_BTN_GAP), MENU_BTN_W, MENU_BTN_H),
            },
        ]
    ...
    def init_solver_scene(self):
        ...
        self.solver_buttons = {
            "start": QRect(642, 182, 270, 50),
            "reset": QRect(642, 246, 270, 50),
            "back": QRect(642, 310, 270, 50),
        }
    ...
    def on_tick(self):
        try:
            ...
            if self.drag_active:
                # Smoothly follow pointer for a more fluid drag feel.
    ...
    def drag_offset_from_click(self, slot_rect, block, pos):
        ...
        # Pick nearest occupied shape cell so drag starts even if click is not pixel-perfect.
    ...
    def apply_solver_setup(self):
        ...
        self.solver_raw.game.hand = [SHAPES[key].copy() for key in self.solver_edit_hand_keys]
    ...
    def try_human_drop(self, slot_idx, anchor_row, anchor_col):
        ...
        if not in_bounds(block, anchor_row, anchor_col):
            self.vs_status = "Invalid placement (outside board)."
            return
    ...
    def ai_reply_in_versus(self):
        ...
        if self.model is None:
            self.vs_a_done = True
            self.vs_status = "No PPO model loaded for AI reply."
            return

        masks = mask_fn(self.vs_a_raw)
        if not np.any(masks):
            self.vs_a_done = True
            self.vs_status = "AI has no legal moves left."
            return

        action, _ = self.model.predict(self.vs_a_obs, action_masks=masks, deterministic=True)
        action = int(action)
        hand_idx, row, col = decode_action(action)
        self.vs_a_obs, reward, self.vs_a_done, _, _ = self.vs_a_env.step(action)
        self.vs_status = f"AI replied with slot {hand_idx + 1} at R{row} C{col} | reward {reward:.1f}"
    ...
    def draw_board(self, painter, grid, board_x, board_y, cell_size, title, accent, ghost_cells=None, ghost_valid=True):
        ...
        self.draw_label(painter, "8 x 8", board_x + 180, board_y - 44, to_color(MUTED), self.body_font)
    ...
    def draw_menu_scene(self, painter):
        ...
        self.draw_label(painter, "Choose one mode to start", 90, 110, to_color(MUTED), self.subtitle_font)

        left_panel = QRect(80, 158, 700, 570)
        right_panel = QRect(812, 158, 790, 570)
        ...
        self.draw_label(painter, self.menu_status, 96, 684, to_color(MUTED), self.body_font)

        self.draw_label(painter, "Modes", 840, 210, to_color(TEXT), self.subtitle_font)
        self.draw_label(painter, "1. Let AI solve your position", 840, 258, to_color(HUMAN), self.subtitle_font)
        self.draw_label(painter, "Build your own board and hand, then let AI continue.", 840, 292, to_color(MUTED), self.body_font)
        self.draw_label(painter, "2. Watch AI play a game", 840, 352, to_color(AI), self.subtitle_font)
        self.draw_label(painter, "Random game start, autoplay with model decisions.", 840, 386, to_color(MUTED), self.body_font)
        self.draw_label(painter, "3. Watch heuristic play", 840, 446, to_color(WARN), self.subtitle_font)
        self.draw_label(painter, "Scores every legal move and always picks the best one.", 840, 480, to_color(MUTED), self.body_font)
        self.draw_label(painter, "4. You vs the AI", 840, 540, to_color((175, 121, 255)), self.subtitle_font)
        self.draw_label(painter, "Split boards. Every valid drop by you triggers one AI move.", 840, 574, to_color(MUTED), self.body_font)

        if self.last_error:
            self.draw_label(painter, self.last_error, 840, 560, to_color(BAD), self.body_font)
    ...
    def draw_solver_scene(self, painter):
        ...
        if self.solver_solving:
            ...
            hand = [SHAPES[k] for k in self.solver_edit_hand_keys]
        ...
        for key, rect in self.solver_buttons.items():
            if key == "start":
                btn = {"rect": rect, "label": "Start AI Solve", "accent": AI}
            elif key == "reset":
                btn = {"rect": rect, "label": "Reset Editor", "accent": WARN}
            else:
                btn = {"rect": rect, "label": "Back To Menu", "accent": BAD}
            self.draw_button(painter, btn)

        self.draw_metric_card(painter, QRect(644, 302, 188, 74), "Stages", str(self.solver_raw.game.stages_passed), HUMAN)
        self.draw_metric_card(painter, QRect(846, 302, 188, 74), "Lines", str(self.solver_raw.game.lines_destroyed), AI)
        self.draw_metric_card(painter, QRect(1048, 302, 188, 74), "Blocks", str(self.solver_raw.game.blocks_placed), WARN)
        self.draw_metric_card(
            painter,
            QRect(1250, 302, 188, 74),
            "State",
            "DONE" if self.solver_done else "LIVE",
            BAD if self.solver_done else GOOD,
        )

        if not self.solver_solving:
            self.draw_label(painter, "Editor controls:", 644, 460, to_color(TEXT), self.subtitle_font)
            self.draw_label(painter, "- Left click board cell: toggle filled / empty", 644, 494, to_color(MUTED), self.body_font)
            self.draw_label(painter, "- Left click hand slot: next shape", 644, 522, to_color(MUTED), self.body_font)
            self.draw_label(painter, "- Right click hand slot: previous shape", 644, 550, to_color(MUTED), self.body_font)
    ...
    def draw_watch_scene(self, painter):
        ...
        new_btn = {"rect": self.watch_buttons["new"], "label": "New Random Game", "accent": WARN}
        back_btn = {"rect": self.watch_buttons["back"], "label": "Back To Menu", "accent": BAD}
        self.draw_button(painter, new_btn)
        self.draw_button(painter, back_btn)

        self.draw_metric_card(painter, QRect(644, 322, 188, 74), "Stages", str(self.watch_raw.game.stages_passed), HUMAN)
        self.draw_metric_card(painter, QRect(846, 322, 188, 74), "Lines", str(self.watch_raw.game.lines_destroyed), AI)
        self.draw_metric_card(painter, QRect(1048, 322, 188, 74), "Blocks", str(self.watch_raw.game.blocks_placed), WARN)
        self.draw_metric_card(
            painter,
            QRect(1250, 322, 188, 74),
            "State",
            "DONE" if self.watch_done else "LIVE",
            BAD if self.watch_done else GOOD,
        )
    ...
    def current_drag_anchor(self):
        ...
        if not in_bounds(block, anchor_row, anchor_col):
            return (anchor_row, anchor_col, False)
    ...
    def draw_drag_piece(self, painter):
        ...
        if anchor_preview is not None:
            # Snap render to exact board anchor while hovering board for precise placement preview.
            ...
            # Free drag render outside board, still aligned with grabbed cell.
    ...
    def draw_versus_scene(self, painter):
        ...
        new_btn = {"rect": self.vs_buttons["new"], "label": "New Match", "accent": WARN}
        back_btn = {"rect": self.vs_buttons["back"], "label": "Back To Menu", "accent": BAD}
        self.draw_button(painter, new_btn)
        self.draw_button(painter, back_btn)
